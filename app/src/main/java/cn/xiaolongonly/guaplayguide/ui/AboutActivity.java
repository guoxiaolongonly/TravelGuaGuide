...
public class AboutActivity extends BaseTitleBarActivity {
    ...
    protected int getLayoutId() {
        return cn.xiaolongonly.guaplayguide.R.layout.activity_about;
    }

    @Override
    protected void init() {
        tvProject = findView(cn.xiaolongonly.guaplayguide.R.id.tvProject);
        tvDataFrom = findView(cn.xiaolongonly.guaplayguide.R.id.tvDataFrom);
        tvIssue = findView(cn.xiaolongonly.guaplayguide.R.id.tvIssue);
        ivIcon = findView(cn.xiaolongonly.guaplayguide.R.id.ivIcon);
    }
...
//        ClickView(tvIssue).subscribe(o -> LaunchUtil.launchDefaultWeb(this, tvIssue.getUrls()[0].getURL(), ""));
        Glide.with(this).load(cn.xiaolongonly.guaplayguide.R.mipmap.ic_launcher)
